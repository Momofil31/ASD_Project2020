...
void computeSolutionFirstInput();

bool dfsFirstInput(int row, int col, int index, int ringCount, int lastWhiteIndex, int lastBlackIndex, char *buffer);
...
bool dfsFirstInput(int row, int col, int index, int ringCount, int lastWhiteIndex, int lastBlackIndex, char *buffer) {
    ...
    if (availableDirection & PATH_OUT_UP) {
        ...
        buffer[index] = 'U';
        isDfsEnd = dfsFirstInput(row - 1, col, index + 1, ringCount, lastWhiteIndex, lastBlackIndex, buffer);
    ...
    if (!isDfsEnd && availableDirection & PATH_OUT_RIGHT) {
        ...
        buffer[index] = 'R';
        isDfsEnd = dfsFirstInput(row, col + 1, index + 1, ringCount, lastWhiteIndex, lastBlackIndex, buffer);
...
#endif
    dfsFirstInput(START_ROW, START_COL, 0, 0, -10, -10, buffer);
}
